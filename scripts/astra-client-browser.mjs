...
try {
  ...
  await page.evaluate(() => {
    ...
    studentCardStatusKnown=true;studentCardStatusError='';
    studentCardStatusContext=JSON.stringify([String(currentSheetName||'').trim(),getScheduleSessionKey()]);
  ...
  await page.waitForFunction(()=>qaCardRequests.length===2);
  await page.evaluate(()=>qaCardRequests[1].resolve({Alice:{studentName:'Alice',sent:false},Bob:{studentName:'Bob',sent:false}}));
  ...
  results.cases.cardMismatchLocksWrites=await page.evaluate(() => Array.from(document.querySelectorAll('#studentImageList .student-list-toggle-btn')).every(button=>button.disabled));
  await page.evaluate(()=>{document.getElementById('studentImageModal').style.display='flex';});
  results.cases.cardErrorVisible=await page.locator('#studentCardStatusNotice').isVisible();
  await page.setViewportSize({width:1440,height:1000});
  await page.screenshot({path:path.join(evidence,'card-error-desktop.png')});
  await page.setViewportSize({width:390,height:844});
  await page.screenshot({path:path.join(evidence,'card-error-mobile.png')});
  await page.evaluate(()=>{loadStudentCardStatuses();});
  await page.waitForFunction(()=>qaCardRequests.length===3);
  await page.evaluate(()=>qaCardRequests[2].resolve({Alice:{studentName:'Alice',sent:false},Bob:{studentName:'Bob',sent:false}}));
  await page.waitForFunction(()=>!studentCardStatusLoading);
  ...
  results.cases.cardErrorCleared=await page.locator('#studentCardStatusNotice').isHidden();
