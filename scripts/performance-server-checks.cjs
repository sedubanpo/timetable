...
ranges = [];
assert.throws(() => scope.setStudentCardSentStatus_('9/17', '학생A', true, 'admin'), /CARD_STORAGE_MOVED/);
assert.equal(rows[1][2], '0', 'old duplicate preserved');
assert.equal(rows[2][2], '0', 'latest archive duplicate preserved');
assert.equal(ranges.length, 0, 'retired write performs no range access');
assert.equal(scope.getStudentCardStatuses_('9/17')['학생A'].sent, false);
assert.throws(() => scope.setStudentCardSentStatus_('9/17', '학생B', true, 'admin'), /CARD_STORAGE_MOVED/);
assert.equal(rows.length, 3);
assert(ranges.every(range => range.width === 5 || range.width === 6));
assert.equal(lockCount, 0);
assert.equal(flushCount, 0);
assert(!locked);
console.log('PASS performance server checks: cold/changed/expired grid 1 read + 1 hash; warm 0; force fresh; teacher/manual edit parity; bounded history exact parity including sparse old matches; projected archive card reads and retired writes without locks.');
