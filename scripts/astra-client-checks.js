...
(async () => {
  ...
  run(`
    currentSheetName='9/4(금)'; studentCardStatusContext=JSON.stringify([currentSheetName,getScheduleSessionKey()]); markStudentCardSentStatus('Alice',true);
    currentSheetName='9/5(토)'; loadStudentCardStatuses();
  ...
  run(`
    ...
    studentCardStatusContext=JSON.stringify([currentSheetName,getScheduleSessionKey()]);
    markStudentCardSentStatus('Alice',true);
  `);
  ...
  await flush();
  assert.equal(run('cardPending[8].method'),'getStudentCardStatuses','failed write must reconcile, not replay');
  assert.equal(run('studentCardStatusLoading'),true,'reconciliation retains save lock');
  run(`cardPending[8].resolve({Alice:{sent:false},Bob:{sent:false}});`);
  await flush();
  ...
  run(`loadStudentCardStatuses();cardPending[9].resolve({Alice:{sent:false},Bob:{sent:false}});`);
  await flush();
  run(`markStudentCardSentStatus('Bob',true);`);
  assert.equal(run('cardPending.length'),11,'next student may save after explicit status refresh');
  run(`cardPending[10].resolve({studentName:'Bob',sent:true});`);
  await flush();
